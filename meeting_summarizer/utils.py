...
def create_messages_from_transcripts(
    ...
    token_limit_per_model = {
        ...
        "text-ada-001": 2000,
        "ada": 2000,
        "gpt-3.5-turbo": 4096,  # Add this line for gpt-3.5-turbo
        "gpt-4": 8192,  # Add this line for gpt-4
        "gpt-4o": 128000,  # Keep your existing gpt-4o if needed
    }

    # Make sure the model is in the token limit dictionary
    if model not in token_limit_per_model:
        raise KeyError(f"Model '{model}' not found in token limit dictionary.")
